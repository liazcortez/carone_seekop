        ...
        onSubmit={async (values, {
          ...
          try {
            ...
            if(selectedActions.length === 0){
              ...
              setErrors({ submit: t("Errors.Comment_can_not_be_more_than_500_characters")});
              ...
              setErrors({ submit: t("Errors.Max3")});
                ...
                if(user && user.role && (user.role === 'rockstar' || user.role === 'admin' || user.role === 'super admin') && comment && comment.user){
                ...
                if(userId === ''){
                  ...
                  if(comment){
                    ...
                    if(user.role === 'admin'){
                      ...
                      await getCommentsByStore(`&store[in]=${getMultiStoresIds(user.stores)}`);
                  ...
                  setDate(moment().endOf('hour').add('1', 'minute'));
        ...
        {({
          ...
          <Card className={classes.root}>
            ...
            <Box p={2}>
              ...
              <Typography variant="h3" color="textPrimary" style={{marginTop: '1em', marginBottom: '1em'}}>
                {t("Titles.CreateTask")}
              </Typography>
              <Typography variant="h4" color="textPrimary" style={{marginTop: '1em'}}> 
                ...
                <List className={classes.flexContainer}>
                  {
                    enumActions.map((action, index) => (
                      <ListItem key={index} onClick={()=>handleSetAction(action)} >
                        <Checkbox checked={selectedActions.includes(action)} onClick={()=>handleSetAction(action)} name={action}/>   
                        <ListItemAvatar>
                          <Avatar className={classes[action]}>{actionIcons[action]}</Avatar>
                        </ListItemAvatar>
                        <Typography color="textPrimary" variant="body1" style={{display: 'block', whiteSpace: 'nowrap', overflow:'hidden'}}>
                      ...
                      >
                        {
                          comment ? 
                          statuses.map((status) => (
                            comment.lead && comment.lead.status && status.name === comment.lead.status.name &&
                              <option key={status._id} value={status._id} name={status.name}>{status && t(`Keys.${status.name}`)}</option>
                          ))
                          : 
                          statuses.map((status) => (
                            lead && lead.status && status.name === lead.status.name &&
                              <option key={status._id} value={status._id} name={status.name}>{status && t(`Keys.${status.name}`)}</option>
                          ))
                        }
                      </TextField>
                    ...
                    >
                      <option value={''} key={0}></option>
                      {
                        substatuses && substatuses.map(item => {
                          if(item.name !== 'rejected' && item.name !== 'new' && item.name !== 'sold' && item.name !== 'frontdesk' && status === item.status){
                            return  <option  key={item._id}  value={item._id} subname={item.name}>{t(`Substatus.${item.name}`)}</option> 
                          }
                          return false;
                        })
                      }
              ...
              <Divider className={classes.divider}/>
              <Box>
                <Reminder label={false} isActive={reminder} setActive={setReminder} startDate={date} setReminderDate={setReminderDate} current={current} setCurrent={setCurrent}/>
              </Box>
              {/* {
