...
import CalendarAtivities from './CalendarAtivities';
...
const DashboardView = ({className, ...rest}) => {
  ...
  const { leads, getLeadsAR, newS, follow, sold, date, loading, getLeadsByUser } = useLead();
  ...
  const [roleUserSearch, setRoleUserSearch] = useState(`&agent=${user._id}`);
  const { getStatuses, statuses } = useStatus();
  const { getAppointmentsAR, appointments } = useAppointment();
  const [arrIds, setArrIds] = useState('')
  const [days, setDays] = useState();

  useEffect(() => {
    setDays(daysAppointment(appointments));
  ...
  useEffect(()=>{
    if(user.role && user.store){
        ...
        setRoleUserSearch(`&agent=${user._id}`);
        getLeadsByUser(user._id, '');
    }

    if((user && user.role) && (user.role === 'rockstar' || user.role === 'super admin' || user.role === 'admin')){
      history.push('/');
    }
    //eslint-disable-next-line
  },[user]);
    ...
    >
      <Container maxWidth={false}>
        ...
        >
            ...
            sm={6}
          >
            <MonthLeads loading={loading.toString()} date={leads.length} title={title}/>
          </Grid>
          ...
          <Grid
            ...
            sm={6}
          >
            <NewLeads loading={loading.toString()} new={newS} 
          title={title}/>
          </Grid>
          ...
          <Grid
            ...
            xs={12}
          >
            <CalendarAtivities appointments={appointments} labels={days}/>
          </Grid>
