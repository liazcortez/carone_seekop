...
const Results = ({ className, makes, ...rest }) => {
  ...
  const handleSelectAllMakes = event => {
    setSelectedMakes(event.target.checked ? makes.map(make => make.ID) : []);
  };
          ...
          <Table>
            ...
            <TableBody>
              {paginatedMakes.map(make => {
                const isMakeSelected = selectedMakes.includes(make.ID);
                  ...
                  >
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        onChange={event => handleSelectOneMake(event, make.ID)}
                        value={isMakeSelected}
                        onClick={e => {
                          setDeletedMakes(false);
                        }}
                      />
                    </TableCell>
                    <TableCell>
                      <Box display="flex" alignItems="center">
                        <div>
                          <Link
                            ...
                            to={`/app/management/makes/${make && make.ID}`}
                            variant="h6"
                          >
                            {make && make.ID}
                          </Link>
                    ...
                    <TableCell>
                      {make && make.Description
                        ? Capitalize(make.Description)
                        : '- - -'}
                    </TableCell>
                    <TableCell>
                      {make && moment(make.createdAt).format('ll')}
                    </TableCell>
                  </TableRow>
                );
              })}
