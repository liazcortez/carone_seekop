...
const ApexChartsView = ({ className, ...rest }) => {
  ...
  useEffect(() => {
    getQuestLeadsAR(`${makeSearch}${storeSearch}${sourceSearch}&after=${moment().subtract(12, 'months').format('YYYY-MM-DD')}&chart=monthlyHC&filter=MMMM-YY`)
  ...
  useEffect(()=>{
    if(contar > 1){
      getQuestLeadsAR(`${makeSearch}${sourceSearch}${storeSearch}&after=${moment().subtract(12, 'months').format('YYYY-MM-DD')}&chart=monthlyHC&filter=MMMM-YY`)
    }
    contar ++;
    //eslint-disable-next-line
      ...
      <Container maxWidth={false}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            ...
            <LineChart leads={chart} type={typeBar} loading={loading} reload={reload}/>
          </Grid>
        </Grid>
      </Container>
    </Page>
